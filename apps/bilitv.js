...
function fail (e, msg, id) {
  if (e.msg != id || !e.msg.includes('bilibili')) {
    return true
  }
...
export class bilitv extends plugin {
  ...
  async jxsp (e) {
    ...
    if (e.msg.includes('点赞' && '投币')) {
      return true
    }
    if (e.msg.match(regB23)) {
      try {
        bvid = regBV.exec(
          (await fetch('https://' + regB23.exec(e.msg)[0].replace(/\\/g, '')))
            .url
        )
        if (bvid == null) {
    ...
    res = await fetch(
      `https://api.bilibili.com/x/player/playurl?avid=${res.data.aid}&cid=${res.data.cid}&qn=16&type=mp4&platform=html5`,
      {
        headers: {
          referer: 'https://www.bilibili.com/',
          'User-Agent':
    ...
    e.reply(
      segment.video(
        Buffer.from(
          await (
            await fetch(res.data.durl[0].url, {
              headers: {
                referer: 'https://www.bilibili.com/',
                'User-Agent':
                  'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Safari/537.36'
    ...
    if (!e.msg.match(regEP)) {
      if (e.msg.match(regMD)) {
        try {
          let temp = await (
            await fetch(
              `https://api.bilibili.com/pgc/review/user?media_id=${regMD.exec(e.msg)[0].replace('md', '')}`,
              {
                headers: {
                  referer: 'https://www.bilibili.com/',
                  'User-Agent':
      ...
      epid = temp.result.main_section.episodes[0].share_url.replace(
